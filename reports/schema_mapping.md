# Schema mapping and judgement calls
...
| Case+whitespace normalisation only | e.g. `Minor`→`MINOR`, `'NORMAL PRODUCTION  '`→`'NORMAL PRODUCTION'` | Lossless. Synonym mapping (e.g. `WELLOPS` vs `WELL OPERATION`, era-2 free-text causes) changes meaning → deferred to an explicit reviewable table (the synonym consolidation below), fed by the failing rows from `cause_category_resolvable`. |
...
regime lost the coded cause taxonomy in practice. This feeds the synonym
consolidation below and the data-quality report.
