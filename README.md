...
— a short client-facing assessment: what the data is, what is wrong
with it, what was done about it, and what a downstream analyst must
...
│   ├── plots.py              the report figures, generated not hand-drawn
│   └── causemodel.py         the NLP cause-recovery experiment
├── tests/                    synthetic-fixture test suite (run in CI)
├── notebooks/01_eda.ipynb    exploration only; nothing load-bearing
└── reports/
    ├── data_quality.md       THE deliverable (client-facing assessment)
    ...
    ├── cause_recovery.md     the NLP experiment: a documented negative result
    └── figures/              PNGs generated by hcr.plots
```
...
```bash
pip install -e ".[dev]"
pytest                # full suite; also run by CI on 3.11 and 3.12
ruff check .
```
